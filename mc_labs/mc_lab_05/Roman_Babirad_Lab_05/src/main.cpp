...
pir_t pirSensor = {D5, false, false};
...
void checkSensor() {
  pirSensor.state = digitalRead(pirSensor.pin) == HIGH;

  if (pirSensor.state && !pirSensor.lastState) {
    pirSensor.lastState = true;
    showNextPage();
  } else if (!pirSensor.state) {
    pirSensor.lastState = false;
  }
}
